...
const HeaderCard = styled.div`
  ...
  color: ${(props) => (props.atrasado ? "#ee5f5f" : "inherit")};
`;
...
const CardTarefa = ({ id_tarefa, servico, status_tarefa, nome_usuario, data, hora, prioridade, horario_conclusao1, conclusao1 }) => {
  ...
  const dataHoraTarefa = moment(`${data} ${hora}`, 'YYYY-MM-DD HH:mm');
  const tarefaAtrasada = (status_tarefa === "aberta" || status_tarefa === "agendado" || status_tarefa === "aguardando") && dataHoraTarefa.isBefore(moment());

  if (nivel === "administrador" || nivel === "administrativo") {
    return (
      <ContainerCard status={status_tarefa} prioridade={prioridade}>
        <HeaderCard atrasado={tarefaAtrasada}>
          {nome_usuario}
        </HeaderCard>
        <Bodycard>
          <p>
