...
const BotaoEditTarefa = ({ id_tarefa }) => {
  ...
  async function getInfos(value) {
    const infos = await fetch(`${Config.backend}/tarefa/2`)
  ...
  async function callTipos(value) {
    const tipos = await fetch(`${Config.backend}:8080/tiposatendimentos`)
      .then((response) => response.json())
  ...
  async function callTecnicos(value) {
    const usuarios = await fetch(`${Config.backend}:8080/selectusuarios`)
      .then((response) => response.json())
